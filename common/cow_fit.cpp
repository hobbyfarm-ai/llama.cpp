...
bool fill_prediction(llama_model * model, llama_context * ctx,
    ...
    auto fill_dev = [&](int idx, ggml_backend_dev_t dev, bool is_host) {
        ...
        if (is_host) {
            ...
            // Match llama.cpp fitting: only non-GPU accelerator backends fall
            // back to host memory when they report an unknown 0/0 budget.
            if (free == 0 && total == 0) {
...
bool pp_fallback_possible(llama_model * model, const llama_context_params & cparams) {
    return cparams.pipeline_parallel_type != LLAMA_PIPELINE_PARALLEL_DISABLED
        && cparams.offload_kqv
        && llama_model_n_devices(model) > 1;
}

bool prediction_fits_budget(const cow_fit_prediction & pred, const size_t * headroom_per_device) {
    for (int i = 0; i <= pred.n_devices; i++) {
        const cow_fit_device_breakdown & d = pred.devices[i];
...
bool predict_with_pp_fallback(llama_model * model,
                              ...
                              const size_t * headroom_per_device = nullptr) {
    bool pp_on_fits = true;
    const bool can_fallback = pp_fallback_possible(model, cparams);
    if (!run_pass(
            model,
            path_model,
            use_mmap,
            cparams,
            out_pred,
            can_fallback && !headroom_per_device ? &pp_on_fits : nullptr)) {
        return false;
    }

    const bool should_try_no_pp = can_fallback
        && (headroom_per_device ? !prediction_fits_budget(out_pred, headroom_per_device) : !pp_on_fits);
    if (should_try_no_pp) {
...
extern "C" cow_fit_max_ctx_result cow_fit_max_ctx(
    ...
    auto fits_at = [&](uint32_t n_ctx, cow_fit_prediction & out_pred) -> bool {
        ...
        cp.n_ctx = n_ctx;
        if (!predict_with_pp_fallback(model, path_model, mparams->use_mmap, cp, out_pred, headroom_per_device)) {
            return false;
        }

        return prediction_fits_budget(out_pred, headroom_per_device);
    };
